...
function register_user($conn)
{
    ...
    if (count($eroor_Array) <= 0) {
        $query = "INSERT INTO `users`(`id`,`employe_id`, `username`, `password`, `image`)
        VALUES('$new_id','$employe_id', '$username',  MD5('$password'),  '$save_name')";
...
function get_user_list($conn)
{
    $data = array();
    $array_data = array();
    $query = "SELECT u.id,e.fullname,u.username,u.image,u.date from users u JOIN employe e on u.employe_id=e.employe_id";
    $result = $conn->query($query);
...
function update_user($conn)
{
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        if (count($eroor_Array) <= 0) {
            $query = "UPDATE users set users.employe_id= '$employe_id',  users.username= '$username', 
        ...
        $query = "UPDATE users set users.emname= '$emmname', users.username= '$username',
        password= MD5('$password') where users.id= '$update_id'";
