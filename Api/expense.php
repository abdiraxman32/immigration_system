...
function get_user_transaction($conn)
{
    ...
    $data = array();
    $array_data = array();
    $query = "SELECT e.id,e.amount,e.type,e.description,u.username,ac.bank_name,e.date FROM expense e JOIN users u on e.user_id=u.id JOIN account ac on e.Account_id=ac.Account_id";
    $result = $conn->query($query);
